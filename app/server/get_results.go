...
func GetResultHandler(params operations.GetResultParams) middleware.Responder {
	results, err := getResults(params.Platform, params.Org, params.Repo, params.Commit)

	if errors.Is(err, errNotFound) {
		return operations.NewGetResultNotFound()
	}
	if errors.Is(err, errInvalidInputs) {
		return operations.NewGetResultBadRequest()
	}
	if err == nil {
		var result models.ScorecardResult
		if err = result.UnmarshalBinary(results); err != nil {
			return operations.NewGetResultOK().WithPayload(&result)
		}
	}

	return operations.NewGetResultDefault(http.StatusInternalServerError).WithPayload(&models.Error{
